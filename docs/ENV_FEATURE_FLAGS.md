...
| `RESTART_CONTEXT_RESTORE_ENABLED` | ⚠ NO | off | **A50** — auto-inject a bounded `<prior_context>` block of recent completed turns when a session's driver was lost on a worker restart and the next task starts a fresh SDK session (`create_session`). Trigger: `session.driver_status == 'lost'` + prior completed turn exists + most-recent turn ≤24h old. Caps: last 3 turn pairs, 1 500 chars/reply, 4 000 chars total. OFF ⇒ no injection, byte-identical. See `TODO(A50-tier2)` in `orchestrator.py` for the Haiku-summariser upgrade path. | `orchestrator.py:_maybe_inject_restart_recovery_context` |

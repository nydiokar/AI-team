...
- **Noted future item (not built):** if model-route-per-node ("cheap DRAFT / strong
  ...
  Phase-2 promotion — out of scope here. It stays COLD — a stated preference applied by
  hand — until a concrete trigger is *observed* (a bad output traced to the wrong model on
  a node, after manual routing was actually tried and recurs). Being named absent here does
  not promote it. (Its formal promotion-ladder doctrine, `promotion_ladder.md`, was retired
  2026-07-06 and deleted 2026-08-01 — the reasoning above stands on its own without it.)

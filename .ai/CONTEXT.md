...
1. **Harness doc drift (A64)** — the XML-packet ritual documented in `docs/harness/` was
   abandoned somewhere around `AGENT_36` (no `<task_packet>` XML in any packet since). The
   `dispatch_pipeline.md` runbook and several generator files point at a workflow that nobody
   has used in weeks. A64 reconciles this: find the real current house style from actual recent
   packets, retire or rewrite stale files, fix the broken pointers. Docs-only, straight to `main`.
...
| **A64** | `AGENT_64_HARNESS_DOC_DRIFT_RECONCILIATION.md` | — | dispatched | Reconcile stale harness docs against real current practice. Docs-only. |

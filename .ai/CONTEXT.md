...
|---|---|---|---|
| **T1** | **M3.4 Job 1 adoption + activation** | The engine exists but is inert: the Manager role still block-polls `wait_for_worker` and the flag is OFF. THIS is the task that delivers the "free-for-convo, woken-on-completion" fix — rewrite the role/dispatch guidance to *arm a group and return control*, thread `round_cap` through `open_case`, then flag-on + restart. | 🟡 **NOT STARTED (small).** Plan §T1. Unblocked (Job 1 merged, PR #45). Live activation = operator-gated. |
| **T2** | **M3.3 completion — turn/cost governor + kill path** | M3.3 shipped only its durable-relay half. `ClaudeAgentOptions` passes NO `max_turns`; no cost cap or kill path runs. A bounded *autonomous* loop is unsafe to run long without this. | 🟡 **NOT STARTED.** Plan §T2. No hard dependency (may run alongside T1 in a different loop). |
| **T3** | **M3.4 Job 2 — durable Case reconstruction** | `get_case_brief` single-call DB state + auto-`reconcile_waits`/re-arm groups at role-boot. The prerequisite that makes crash-respawn safe. | 🟢 **DESIGN-READY.** Plan §T3 / design §7 job 2. ⛔ T1 adoption. |
| **T4** | **M3.4 Job 3 — crash-respawn** | Respawn a role-full Manager on a dead-session Case (reconstruct via T3, re-arm, resume). Closes the "survive a process restart" clause of the goal. | 🟢 **DESIGN-READY.** Plan §T4 / design §7 job 3. ⛔ T3. |
| **T5** | **M4 — spec authoring + scored review + decomposer-as-task-DAG** | The feature-sized-intent front-end: author a spec, rubric-score it, then decompose into a task-DAG inside ONE Case. Currently absent entirely. | 🟢 **NOT STARTED.** Plan §T5. Generators can precede; wiring ⛔ T1. |
| **T6** | **M4 hybrid-executor spike (gated)** | Decide invoke-vs-build for the intra-task parallel executor (SDK Workflows/subagents). Go/no-go + mechanism, not a full build. | ⚪ **SPIKE.** Plan §T6. ⛔ T4. |

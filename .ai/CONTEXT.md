...
|---|---|---|---|---|
| **A82** | `AGENT_82_SESSION_TURN_QUEUE.md` | — | active — Stage 3 in review (A87) | Unified durable turn queue on `feat/session-turn-queue` (worktree `~/dev/AI-team-wt/a82`, never merged until reviewed). Stages 1–2 ACCEPT; Stage 3 reworked twice, adversarial review pending. End state = ONE pathway: new Stage 8 cutover deletes protocol 0 (see A87 rulings). **Deferred (§7 trace):** an oversize managed result enters `recovery_required` with a bounded reason, but the full output is not persisted to carrier artifact storage (lives only in worker memory until the handler exits) — must be closed before Stage 8 deletes the legacy path. |

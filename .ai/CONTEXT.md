...
| 36 | **Watched jobs notify user + agent in-session** — ✅ terminal watched jobs are projected into the owning session via canonical `mesh_tasks` rows + session history fallback; `notify_agent=1` submits a `watched_job` follow-up instruction with `job_id` metadata so the agent can continue. WebUI visibility: System → Jobs shows an `agent` chip when `notify_agent` is set; Session → Chat result includes `Agent continuation requested.` | `CONTEXT.md` | Backend + Frontend ✅ |

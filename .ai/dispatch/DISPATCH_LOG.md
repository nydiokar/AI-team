...
| A25 | `AGENT_25_FLOW_RELATIONSHIP_SCHEMA.md` | 2026-07-08 | 3 | dispatched | **Work Control Substrate 1/5.** Add durable `flow_links` + append-only `flow_events` schema/helpers so Work/Cases can be built from authoritative relationships, not heuristics. |
| A26 | `AGENT_26_FLOW_LINK_WRITE_PATH.md` | 2026-07-08 | 3 | dispatched | **Work Control Substrate 2/5.** Populate flow links/events at safe existing seams (flow start, task dispatch, approvals, session roles where known) without changing execution behavior. **Depends on A25 + A26a; CONSUMES A26a's stamped parent linkage — no second stamping hook** (see packet de-confliction note). |
| A27 | `AGENT_27_WORK_READ_MODEL_API.md` | 2026-07-08 | 2 | dispatched | **Work Control Substrate 3/5.** Read-only Work/Case projection API over flows, links, events, tasks, approvals, sessions, artifacts, jobs with honest partial/unlinked coverage. |
| A28 | `AGENT_28_MOBILE_WORK_SURFACE.md` | 2026-07-08 | 2 | dispatched | **Work Control Substrate 4/5.** Mobile Work tab: attention/active cases, linked evidence, compact graph/navigation, and session affiliation labels; no creation, no workflow editor, no mutation. |
| A29 | `AGENT_29_WORK_SUBSTRATE_HARDENING.md` | 2026-07-08 | 2 | dispatched | **Work Control Substrate 5/5.** Integrated adversarial review, stale/conflict/no-heuristic fixtures, docs closure, and final milestone acceptance. |
| A30 | closure §A30 in `docs/WORK_CONTROL_SUBSTRATE_MILESTONE.md` (no separate packet — post-closure polish) | 2026-07-09 | 2 | built | **Post-closure truthfulness polish** on `feat/work-surface-a28-a29`. Re-reviewed A28+A29 write-seams (all sound). Fixed 2 doc/label-truthfulness defects: docstring+hook comment lied "oldest link" (actual = most-recent, `list_session_case_links` is newest-first); affiliation label now surfaces authoritative `caseStatus` so a **closed** case reads as muted history (not implied active work) via `isClosedCaseStatus` mirroring backend `_CLOSED_STATUSES`. Additive/read-only; 37 backend + 93 vitest green, build OK, gateway untouched. |

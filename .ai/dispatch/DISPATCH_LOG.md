...
| A12 | Harness self-test — run one real task through the loop | `AGENT_12_HARNESS_SELFTEST.md` | `feat/task-harness` → main (**PR #8**) | **merged** | Ran the §14 loop by hand on `docs/harness/dispatch_pipeline.md`: sharpened the two-lane scope banner (`.task.md` batch vs live `submit_instruction`) and replaced the terse worked example with a copyable all-7-stage example (real filled packet + closed milestone + 2 F-tags + closure). Docs-only; all cross-refs verified. HARNESS FRICTION REPORT appended to the packet with the Phase-2 Y/N verdict. | **OPERATOR: read the friction report** in `AGENT_12_HARNESS_SELFTEST.md`, then decide merge. No code, no gateway state. |
| A13 | WebUI-first surfacing of the Level-3 admission block (A9H "Next") | `AGENT_13_HARNESS_BLOCK_SURFACE.md` (+ `.milestone.md`) | `feat/harness-block-surface` | **built, awaiting operator merge** | The Web lane now translates `HarnessAdmissionBlocked` into a clean **409** (`reason=harness_level3_needs_approval` + human `detail` + echoed `task_id`) instead of an opaque 500; new `session_service.mark_idle` reverts the optimistically-BUSY session so it isn't stranded; `Composer.tsx` shows the approval-needed copy instead of the misleading "tap send to retry". Gate predicate UNTOUCHED; guard OFF ⇒ byte-identical. ZERO new gateway state. 2 new write-surface tests; 64 control-API + 25 session-service green; 689 collect clean; web typecheck clean. Self-review: 5 F-tags, no P0/P1 (idempotency lock/cache verified safe on the raise path). | **OPERATOR: merge decision.** Review `feat/harness-block-surface` diff, then → `main` if satisfied. **Follow-ups (out of scope):** Telegram reply wiring for the same signal; approve-from-UI workflow (deferred, CONTEXT #25). |

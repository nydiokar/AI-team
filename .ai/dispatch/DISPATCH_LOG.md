...
| A26a | `M2_DISPATCH_LINEAGE_WIRING.md` (branch `feat/m2-dispatch-lineage-wiring`) | 2026-07-08 | 3 | built — op-merge | **v0.6 M2 — flow_runs lineage half (the "wiring-only" piece).** Stamp + persist mig-22 lineage cols (`parent_flow_run_id`/`dispatched_by`/`dispatch_file`) at the child-dispatch seam behind `HARNESS_FLOW_DRIVE` (default OFF ⇒ byte-identical); adds `_stamp_child_dispatch_lineage` supplier + `list_child_flow_runs` reverse-lookup. **Supplier half that A26 consumes — NOT a duplicate of `flow_links`.** 19/19 tests green (`tests/test_dispatch_lineage.py`). Reconciles the overlap w/ A26. |

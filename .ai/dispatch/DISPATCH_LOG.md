...
| A24 | `AGENT_24_DECOMPOSER_GENERATOR.md` | 2026-07-07 | 2 | dispatched | **v0.6 M4 (generators-early split).** Docs-only: author `docs/harness/generators/decomposer.md` — MAX salvage of the TaskExpert decomposition pattern (see the MAX salvage map in `docs/`) as a by-hand DRAFT front-half: intent → N dependency-aware packets via existing `continues:`/lineage cols. No code, no wiring (M3 later), no auto-approve. Independent of A20–A23. |

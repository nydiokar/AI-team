...
| A11 | Fix: mesh affinity routing — session pin ignored at execution (silent local fallback) | `AGENT_11_MESH_AFFINITY_ROUTING.md` | `feat/task-harness` (operator: no new branch) | **built — awaiting kanebra redeploy + T3 re-validation** | Diagnosed via kanebra reads (no paid turn): H1/H2 ruled out — `MESH_ENABLED=true`, `sessions.machine_id=Horse`, 1 mesh_tasks row, **0 remote-dispatch log hits**. Real cause: gateway's in-process worker pool ran the Horse-pinned task locally; `route_remote` was False at the decision site and there was **no affinity guard in the local path**, plus `SessionStore.create()` transiently stamped the local hostname before the pin. **Fix (2 parts):** (1) `create()` accepts `machine_id` → atomic pin, no local-host window (`session_store.py`/`session_service.py`); (2) hard affinity guard in `process_task` — refuse local execution of a remote-pinned session, emit `event=affinity_unrouted`, force remote when mesh on / honest-fail when mesh off (`orchestrator.py`). 2 new regression tests; **54 passed** (session/lifecycle/control-api/mesh-dispatch), no CLI. | **Operator:** redeploy gateway on **kanebra** (fix lives in the gateway process). **T3:** re-run A10 §T1 smoke (1 turn) → expect `gateway_node_id=kanebra` + `execution_node_id=Horse` distinct; then mark A10 §T1 PASSED + update CONTEXT/A10 docs. Watch for `affinity_unrouted` in the log = still-unrouted (node offline/misregistered), don't mark passed. |

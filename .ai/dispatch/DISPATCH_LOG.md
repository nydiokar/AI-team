...
| A35 | `AGENT_35_LIVE_F4_SPIKE.md` (branch `feat/m3-phase30-mcp-manager`) | 2026-07-10 | 3 | dispatched — op-gated | **M3 Phase 3.0 acceptance (the paid live proof).** Operator runbook: register manager server + `MANAGER_TOOLS_ENABLED=1` + gateway restart → spawn a manager session → `dispatch_worker`→`wait_for_worker` → verify parent→child lineage in `/api/flows` + no task-slot starvation, with exact commands, health checks (`curl :9003/health`, never `python main.py status`), and full rollback. **NOT executed autonomously** — paid CLI (scar #1) + live PM2 restart (Ask First) + global-config mutation. Awaiting operator go/no-go. |
